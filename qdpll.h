...
/* Open a new scope at the right end of the quantifier prefix, where variables
   can be added by 'qdpll_add'. Adjacent scopes of the same quantifier type
   can be added but they will be merged into a single scope. The opened scope
   must be closed by adding '0' via 'qdpll_add'. Returns the nesting of the
   added scope, which should be used as a handle of this scope, and which can
   safely be passed to 'qdpll_add_var_to_scope'. 
...
/* Open a new scope at nesting level 'nesting >= 1' with quantifier type
   ...
   scope must be closed by adding '0' via 'qdpll_add'. Adjacent scopes of the
   same quantifier type can be added but they will be merged into a single
...
/* Add variables or literals to clause or opened scope. Scopes are opened by
   ...
   'qdpll_push' has NOT been called before then the added clauses are permanently
   added to the formula. Otherwise, they are added to the current frame and can
   be remove from the formula by calling 'qdpll_push'.
   ...
   and put in the leftmost scope. See also function 'qdpll_is_var_declared'
   below. */
...
/* Returns non-zero if and only if (1) a variable with ID 'id' has been added
   ...
   useful for incremental usage of the solver: before adding a clause to the
   solver, the function can be used to check if the variable of each literal
